 ...
 * v2.1 – BLE dashboard, 2-step WHITE/BLACK calibration 500 samples
 *
...
void handleInputs() {
  ...
  // Sửa lỗi: sau khi lấy WHITE xong và đang chờ BLACK,
  // bấm ngắn hay giữ lâu đều PHẢI lấy BLACK, không được quay lại WHITE.
  //
  // CAL_IDLE + giữ lâu            : bắt đầu calib WHITE x500
  // CAL_WAIT_BLACK + bấm/giữ      : lấy BLACK x500
  // CAL_IDLE + đã calib + bấm ngắn: RUN/STOP
  if (ev != 0) {
...
void setup() {
  ...
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);
  display.setCursor(14,  2); display.print("=== LINE BOT v2.1 ===");
  ...
  display.setCursor(0,  48); display.print("Hold CAL: WHITE x500");
  display.setCursor(0,  58); display.print("Then CAL: BLACK x500");
  display.display();
  delay(2500);
}
